...
//                .setPaddings(-1, 0, -1, 0)
        ...
        List<HotelFasilitasDto> hotelFasilitasDtoList;
        try {
            hotelFasilitasDtoList = objectMapper.readValue(transactionDto.getDetail().get("hotel").get("facilities").traverse(), new TypeReference<>() {
            });
        } catch (IOException e) {
            throw new RuntimeException(e);
        }
        ...
        for (int count = 0; count <= 10; count++) {
            ...
            fasilitasText.append(fasilitasTextPart);
        }
        StringBuilder sb = new StringBuilder(fasilitasText.toString());
