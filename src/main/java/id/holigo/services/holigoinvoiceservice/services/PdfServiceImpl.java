...
public class PdfServiceImpl implements PdfService {
    ...
    public void export(HttpServletResponse response, TransactionDto transactionDto)
        ...
        String titleLbl = "";
        String subtitleLbl = messageSource.getMessage("invoice.description", null,
                LocaleContextHolder.getLocale());
        String invoiceNumberLblText = messageSource.getMessage("invoice.number", null,
                LocaleContextHolder.getLocale());
        String invoiceDateLblText = messageSource.getMessage("invoice.date", null,
                LocaleContextHolder.getLocale());
        ...
        String getTotalFareText = messageSource.getMessage("invoice.rupiah", null,
                LocaleContextHolder.getLocale())
                + " ";
        String infoText = messageSource.getMessage("invoice.tax-included", null,
                LocaleContextHolder.getLocale());
        ...
        switch (transactionDto.getTransactionType()) {
            ...
            case "PAM" -> {
                titleLbl = messageSource.getMessage("invoice.postpaid-pdam", null,
                        LocaleContextHolder.getLocale());
                getFareText += getPrice(transactionDto.getFareAmount());
                serialNumberText = transactionDto.getDetail().get("standMeter").asText();
                getTotalFareText += getPrice(transactionDto.getFareAmount());
                serialNumberLbl = "invoice.stand-meter";
            }
            case "GAS" -> {
                titleLbl = messageSource.getMessage("invoice.postpaid-gas", null,
                        LocaleContextHolder.getLocale());
                getFareText += getPrice(BigDecimal.valueOf(transactionDto.getDetail().get("billAmount").asDouble()));
                serialNumberText = getPrice(BigDecimal.valueOf(transactionDto.getDetail().get("adminAmount").asDouble()));
                getTotalFareText += getPrice(transactionDto.getFareAmount());
                serialNumberLbl = "invoice.admin-fee";
            }
            case "DWAL" -> {
        ...
        // Holigo image
        Image holigo = Image.getInstance("https://ik.imagekit.io/holigo/Icon/image-3_MdrlPTotGl.png?ik-sdk-version=javascript-1.4.3&updatedAt=1658551543582");
        holigo.setAbsolutePosition(50, 750);
        holigo.scaleAbsolute(90, 40);
        cb.addImage(holigo);

        // Holigo Text
//        Font holigoFont = new Font(bf, 18, Font.BOLD, new Color(0, 189, 23, 1));
//        Chunk holigoText = new Chunk(
//                messageSource.getMessage("invoice.app", null, LocaleContextHolder.getLocale()));
...
//        holigoCt.go();
        ...
        ct.addText(new Phrase(40,
                ...
                detailLbl));
        ct.addText(new Phrase(40,
                messageSource.getMessage("invoice.payment-method", null,
                        LocaleContextHolder.getLocale()) + "\n",
                detailLbl));
        ct.addText(new Phrase(40,
        ...
        ct.addText(new Phrase(40,
                messageSource.getMessage("invoice.additional-information", null,
                        LocaleContextHolder.getLocale())
                        + "\n",
                detailLbl));
        ct.go();
        ...
        Chunk fareAmountLbl = new Chunk(messageSource.getMessage("invoice.total-fare", null, LocaleContextHolder.getLocale()));
        ...
        fareAmountValueCt.setSimpleColumn(305, endingPoint - 10, 400, endingPoint - 50, 40,
                Element.ALIGN_RIGHT);
